...
import com.example.user.registerusers.DB.DBHanlder_Anko
import com.example.user.registerusers.DB.UserInfo
...
class SaveUserActivity : AppCompatActivity() {
    ...
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<out String>, grantResults: IntArray) {
        var notGranted = arrayOfNulls<String>(permissions.size)
        when(requestCode)
        {
            REQ_PERMISSION->{
                var index:Int= 0
    ...
    {
        val user:UserInfo= UserInfo(edit_name.text.toString(), edit_tel.text.toString(),
        edit_email.text.toString(),mSelectedImgId.toString())
